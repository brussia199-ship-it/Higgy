...
async def admin_create_check_start(call: types.CallbackQuery, state: FSMContext):
    ...
    await Form.waiting_gift_for_check.set()
    await call.answer()

@dp.callback_query_handler(lambda c: c.data.startswith("gift_"), state=Form.waiting_gift_for_check)
...
@dp.callback_query_handler(lambda c: c.data == "mailing_send", state=Form.waiting_mailing_text)
...
@dp.callback_query_handler(lambda c: c.data == "mailing_cancel", state=Form.waiting_mailing_text)
