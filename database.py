...
def add_seller_request(user_id, username):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO seller_requests (user_id, username) VALUES (%s, %s)", (user_id, username))
    conn.commit()
    cursor.close()
    conn.close()
...
def get_active_deal(buyer_id, product_id):
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        "SELECT * FROM deals WHERE buyer_id = %s AND product_id = %s AND status = 'active'",
        (buyer_id, product_id)
    )
